...
public class PostDominators extends ProgramVisitor {
  public static void computeAll(Program program) {
    PostDominators computer = new PostDominators();
  ...
  public Set<ProgramPoint> compute(CFG cfg, ProgramPoint pp) {
    ...
    for (Edge edge : cfg.getOutgoingEdges((Statement)pp)) {
      ProgramPoint peer = edge.getDestination();
